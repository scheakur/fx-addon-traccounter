...
numbersPanel.port.on('remove', function(id) {
  if (id !== '') {
    var index = storage.urls.map(function(info) {
      return info.id;
    }).indexOf(id);
    if (index > -1) {
...
numbersPanel.port.on('move', function(from, to) {
  var target = storage.urls.splice(from, 1)[0];
  storage.urls.splice(to, 0, target);
});
