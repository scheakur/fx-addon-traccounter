...
function getCountUrl() {
  return 'http://trac.edgewall.org/report/1';
}
...
function scheduleRequest() {
  let delay = Math.floor(Math.random() * (POLL_INTERVAL_MAX - POLL_INTERVAL_MIN)) + POLL_INTERVAL_MIN;
  timers.setTimeout(polling, delay);
}

function listUrls() {
  return Object.keys(ss.storage.urls);
}
